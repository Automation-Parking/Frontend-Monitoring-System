...
const Button = (props) => {
  const { children, variant, background, type = "button" } = props;
  return (
    <button
      type={`${type}`}
      class={`${variant} bg-${background}-700 hover:bg-${background}-800 focus:ring-4 focus:ring-${background}-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:${background}-600 dark:hover:bg-${background}-700 focus:outline-none dark:focus:ring-${background}-800`}
    >
      {children}
    </button>
  );
